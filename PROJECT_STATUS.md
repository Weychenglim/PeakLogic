...
## Purpose
This file is the current operational snapshot for PeakLogic. Future threads should read this file first to understand where work stands, what is locked, and what to do next.
...
- Tightened production high MD-risk chart markers so `is_peak_risk_overlay` now requires both a high overlay score and upper-decile MD-risk envelope load, reducing non-actionable lower-load evening callouts.
...
- Added a React-side Optimization Explainable AI panel under Options Considered that turns tested scenario outputs into simple comparison evidence: why cheaper options lost, why larger options show weaker incremental return, what dispatch sample supports the battery/PV split, and which inputs can change the ranking.
- Added a floating read-only Dashboard Assistant for active analyses. React sends compact dashboard context to FastAPI, FastAPI answers through deterministic grounded logic by default, and optional AI provider routing stays behind server-side environment variables with general `AI_ASSISTANT_*` config.
- Enlarged the Dashboard Assistant panel, added per-answer API mode versus dashboard data mode badges, and cleaned markdown-style provider output before rendering so raw `##`, `**`, and bullet markers do not show in chat.
- Added structured Dashboard Assistant suggested actions. Each answer now surfaces app-owned navigation buttons to relevant tabs such as Site Profile, Forecast & Risk, Optimization, Executive Summary, or Settings.
- Added Forecast & Risk top-window evidence to the Dashboard Assistant context and routed high-risk event/window questions to that evidence before generic approval-risk checks, so questions like "most high risk events" answer with ranked forecast windows instead of CAPEX assumptions.
- Added a broader next-step fallback route for Dashboard Assistant questions such as "what should I do" and "step by step guide", producing an action plan instead of repeating the generic site overview.
...
- Added public demo deployment configuration for Render backend and Vercel frontend, including Vercel CORS support and environment-variable guidance.
- Wired the sidebar New Analysis button to return to Data Upload and added a simple Settings page for deployment/API/Supabase status.
...
- Updated README and root project documentation language so the application name is PeakLogic; legacy names are retained only where they identify current folder paths or package/module names.
- Updated README with a first-section project description covering how PeakLogic works and its main functions, and removed the Public Demo Deployment section.
...
- Test the deployed Render/Vercel public demo end to end after the Render URL and Vercel URL are known.
...
- The Dashboard Assistant is explanatory only. It should not change assumptions, rerun analysis, approve recommendations, or default to judge-facing presentation scripts.
- Assistant provider credentials should use `AI_ASSISTANT_API_KEY`, `AI_ASSISTANT_BASE_URL`, and `AI_ASSISTANT_MODEL` where possible; `ZAI_*` aliases are supported for the current Z.AI-compatible chat completions setup.
- Backend assistant secrets belong in the root `.env` copied from `.env.example`; the frontend `kinetic-precision/.env.example` is intentionally limited to Vite-safe public settings.
- Assistant answers show their source mode in the message bubble: "API mode" means a provider returned the answer, while "Dashboard data mode" means deterministic fallback answered from the active dashboard context.
- Assistant suggested actions are generated from deterministic allow-listed app logic, not free-form LLM navigation text.
...
- Render free-tier cold starts may slow the first API request during public demos.
...
- 2026-06-11: Replaced the repetitive Optimization "Why this one" copy with a data-driven Explainable AI panel placed under Options Considered. React now builds comparison evidence from scenario alternatives, optimized dispatch samples, and sensitivity inputs so the section explains model reasoning instead of restating visible KPIs.
- 2026-06-11: Added the Dashboard Assistant feature. It appears only when an analysis exists, answers active-analysis questions from compact context, excludes judge-script prompts, and keeps optional provider API keys on the FastAPI side. Generic OpenAI-compatible chat completion settings and `ZAI_*` aliases are supported, with deterministic fallback if provider calls fail.
- 2026-06-12: Added suggested navigation actions to each Dashboard Assistant answer. The backend returns allow-listed target tabs and the frontend renders them as buttons that switch users to the relevant dashboard section.
- 2026-06-12: Fixed Dashboard Assistant high-risk event answers. React now passes Forecast & Risk Top Risk Windows into the assistant context, and FastAPI routes high-risk event/window questions to those ranked windows before generic assumption-risk fallback.
- 2026-06-12: Added Dashboard Assistant next-step handling so practical guide questions return a short action plan grounded in forecast windows, selected assets, optimized MD, annualized savings, assumptions, and data quality.
- 2026-05-21: Added `render.yaml`, `kinetic-precision/vercel.json`, Vercel CORS support, and deployment instructions for a Render + Vercel public demo.
- 2026-06-02: Tightened Forecast & Risk peak-risk marker selection so lower-load evening uncertainty points remain scored internally but are not chart-highlighted unless they are also materially high MD-risk loads. Added a bundled Site 3 regression for the Mar 3 midday peak versus 8 PM lower-load point.
